...
{ 
    ...
    uut = new VTx_top;   // Create instance
    ...
    uut->start = 0;
    //uut->reset = 0;
    uut->i_data = 0x9A;   // 8-bits binary : 1001_1010
    uut->clk = 0;
    uut->eval();
...
#define Tx_STOP_BIT 11
	...
	if ( number_of_sampling_clocks_passed*RX_SAMPLING_PERIOD - time_sampling == RX_SAMPLING_PERIOD ) {  // always @(posedge sampling_clock) 
		time_sampling = number_of_sampling_clocks_passed*RX_SAMPLING_PERIOD;  // to emulate /*time_ps is exact multiples of RX_SAMPLING_PERIOD*/)
		//cout << "time_sampling = " << time_sampling << endl;
